...
export async function runChecks({ port = config.port } = {}) {
  ...
  if (!fs.existsSync(SETTINGS)) {
    add('Claude Code hook 등록', 'fail', `${SETTINGS} 없음`, '`claude-controller install-hooks` 실행');
  } else {
  ...
  if (settings) {
    ...
    if (!ours) {
      add('Claude Code hook 등록', 'fail', 'PermissionRequest hook에 hook-handler가 없음', '`claude-controller install-hooks` 실행');
    } else {
      add('Claude Code hook 등록', missing.length ? 'warn' : 'ok', missing.length ? `누락 이벤트: ${missing.join(', ')}` : `${EVENTS.length}개 이벤트 등록됨`,
        missing.length ? '`claude-controller install-hooks` 재실행' : undefined);
      ...
      if (handler && fs.existsSync(handler) && path.resolve(handler) !== path.resolve(src) && fs.existsSync(src)) {
        ...
        add('hook-handler 최신 여부', same ? 'ok' : 'warn', same ? '복사본이 현재 버전과 일치' : '복사본이 현재 버전과 다름', same ? undefined : '`claude-controller install-hooks` 재실행');
      }
      const m = /CLAUDE_CONTROLLER_PORT=(\d+)/.exec(ours.command);
      const hookPort = m ? Number(m[1]) : 9200;
      if (hookPort !== port) add('hook 포트', 'fail', `hook은 ${hookPort}, 데몬 설정은 ${port}`, '`claude-controller install-hooks` 재실행으로 포트를 맞추세요');
      const timeout = ours.timeout ?? 0;
      if (timeout < config.permissionWaitSeconds) add('hook 타임아웃', 'warn', `PermissionRequest hook timeout ${timeout}s < permissionWaitSeconds ${config.permissionWaitSeconds}s`, 'hook 타임아웃이 먼저 끊어 passthrough 됩니다 — install-hooks 재실행');
    }
  }
