...
3. **폰-맥 연결은 USB 직결** — Wi-Fi를 쓸 수 없는 환경(AP 격리망 등)에서도 동작.
   유심 없는 공기계는 안드로이드(adb reverse)만 해당하고, 아이폰은 개인용 핫스팟이 켜져야
   하므로 유심/데이터 요금제가 필요하다. 안드로이드는 `adb reverse tcp:9200`, 아이폰은 USB 테더링 인터페이스
